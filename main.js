...
async function isUserSubscribed(userId) {
  try {
    ...
    logger.error(`Ошибка проверки подписки для ${userId}: ${error.message || error}`);
    return false;
  }
}
...
async function sendPdf(chatId, name) {
  ...
  if (cachedPdfFileId) {
    ...
    const msg = await bot.telegram.sendDocument(
      ...
      Input.fromLocalFile(
        PDF_PATH,
        "Чек-лист_ТОП-10_ошибок_конверсии.pdf"
      ),
      {
        caption: "📎 Твой чек-лист готов. Приятного изучения!",
        parse_mode: "HTML",
...
async function main() {
  ...
  logger.info(`Run polling for bot @${me.username} id=${me.id} - '${me.first_name}'`);
}

main().catch((error) => {
  logger.error(`❌ Критическая ошибка запуска: ${error.message || error}`);
  process.exit(1);
});
